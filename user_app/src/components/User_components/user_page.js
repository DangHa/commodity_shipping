...
export default class User extends Component {
  ...
  async componentDidMount() {
    ...
    var result = await fetch('http://172.18.0.1:8080/user/getInfoUser', data)
  ...
  async sendNewInformation() {
    ...
    var result = await fetch('http://172.18.0.1:8080/user/updateInforUser', data)
            .then((response) => response.json())
            .then((json) => {
                return json
            })
    ...
    return (
      ...
      <View style={styles.container}>
        <View style={{alignItems: 'center'}}> 
          <Text style={[{fontSize: 32, color: 'tomato'}, {fontWeight: "bold"}]}>Your Information{"\n"}</Text>
        </View>
